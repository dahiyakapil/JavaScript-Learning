/*
    ...
    2. Whenever an execution context is created, a lexical environment is also created. Lexical environment is the local memory along with the lexical environment of its parent. Lexical as a term means in hierarchy or in a sequence.
...
function parent() {
  console.log(a); // fisrtly undefined in execution context then it finds the value in gloabl scope
  var b = 20;
  child();
  function child() {
  ...
  console.log(x);
  // not defined in local memory, it will search in its own local memory and as it is not present in its own local memory then it will search its parent lexical env that is the global scope and the variable x is present in the global scope. So it will print the variable in the console.
}
